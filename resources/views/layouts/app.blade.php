...
<head>
    ...
    <title>@yield('title', 'Dashboard') · Sistem Pengajuan Transaksi Pengeluaran</title>
...
<div class="app-shell">
    <aside class="sidebar" id="sidebar">
        ...
        @if($role === 'staff')
            <a href="{{ route('pengajuan.index') }}" class="side-link {{ $current==='pengajuan.index' ? 'active':'' }}"><i class="bi bi-clock-history"></i> Riwayat Pengajuan</a>
            <a href="{{ route('pengajuan.create') }}" class="side-link {{ $current==='pengajuan.create' ? 'active':'' }}"><i class="bi bi-file-earmark-plus"></i> Buat Pengajuan</a>
        @elseif(in_array($role, ['spv','manager','direktur']))
            <a href="{{ route('approval.index', $role) }}" class="side-link {{ $current==='approval.index' ? 'active':'' }}"><i class="bi bi-check2-square"></i> Antrian Approval</a>
        @elseif($role === 'finance')
            <a href="{{ route('finance.index') }}" class="side-link {{ $current==='finance.index' ? 'active':'' }}"><i class="bi bi-wallet2"></i> Antrian Finance</a>
        @endif

        <div class="sidebar-footer">
            <div class="user-chip">
                <div class="avatar-badge">{{ strtoupper(substr(auth()->user()->name,0,1)) }}</div>
                <div>
                    <div class="name">{{ auth()->user()->name }}</div>
                    <div class="role">{{ $roleLabels[$role] ?? $role }}</div>
                </div>
            </div>
